...
<html>
  <head>
	...
	<script type="text/javascript">
		...
		function fnSubmit()
		{
			...
			document.adminParamHLForm.action = "/ttcs/adminParamHL.spr?btnPressed=D" ;
			document.getElementById("btnPressed").value = 'D';
			document.getElementById("adminParamHLForm").submit(); 
		}
  ...
  <body onload="fnOnload();">
	...
	<form:form commandName="adminParamHLCommand" id = "adminParamHLForm" name = "adminParamHLForm" action="adminParamHL.spr" method="POST" >
		<div id="NewCustomerDiv"  >
   			<table class="labelandfield"  class="list" style="margin-left: 10%;margin-right: 10%;background-color: white;">
   				<tr>
   					<td class="subhd" ><div id="mainHeaderid" style="border-bottom: 2px solid #ff9900;widows: 95%;margin-left: 2%">Hand Loan Account Parameter Setup</div></td>
   				</tr>
   				<tr>
   					<td width="100%">
   						<div class="section">
   							...
   							<table width="100%">
								...
								<tr>
									...
									<td> <spring:bind path="adminParamHLCommand.depositPcnt"> 
									...
									<td> <spring:bind path="adminParamHLCommand.rateOfInterest"> 
										  <input type="text" name="<c:out value="${status.expression}"/>"
											id="<c:out value="${status.expression}"/>"
											value="<c:out value="${status.value}"/>"  size="30"  maxlength="40" />
									     </spring:bind>
								...
								<tr>
									...
									<td> <spring:bind path="adminParamHLCommand.numberOfinstallments"> 
										  <input type="text" name="<c:out value="${status.expression}"/>"
											id="<c:out value="${status.expression}"/>"
											value="<c:out value="${status.value}"/>"  size="30"  maxlength="40" />
									     </spring:bind>
									...
									<td> <spring:bind path="adminParamHLCommand.processingCharge"> 
										  <input type="text" name="<c:out value="${status.expression}"/>"
											id="<c:out value="${status.expression}"/>"
											value="<c:out value="${status.value}"/>"  size="30"  maxlength="40"/>
									     </spring:bind>
								...
								<tr>
									...
									<td> <spring:bind path="adminParamHLCommand.latePaymentStdFee"> 
										  <input type="text" name="<c:out value="${status.expression}"/>"
											id="<c:out value="${status.expression}"/>"
											value="<c:out value="${status.value}"/>"  size="30"  maxlength="40"/>
									     </spring:bind>
									...
									<td> <spring:bind path="adminParamHLCommand.latePaymentVarFee"> 
										  <input type="text" name="<c:out value="${status.expression}"/>"
											id="<c:out value="${status.expression}"/>"
											value="<c:out value="${status.value}"/>"  size="30"  maxlength="40"/>
									     </spring:bind>
